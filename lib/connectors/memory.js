...
Memory.prototype.beginTransaction = function(isolationLevel, cb) {
    var connector = this;
    debug('Begin a transaction with isolation level: %s', isolationLevel);
    var tx = new Transaction(connector, null);
    cb(null, tx);
};

Memory.prototype.commit = function(connection, cb) {
    debug('Commit a transaction');
    cb(null);
};

Memory.prototype.rollback = function(connection, cb) {
